...
export const LeaderboardContent: React.FC = () => {
  const { data, loading, error } = DashBoard.useContainer();
          ...
          : <>
            ...
            <RankingList>
              <ul>{data?.users.map((u, i) =>
                <UserRow key={u.id}>
                  <Text color="grey.400" fontSize="xsm">{i + 1}</Text>
                  <UserProfile user={u} />
                  <Text color="grey.600" fontSize="xsm">{u.firstName} {u.lastName}</Text>
                </UserRow>)}
              </ul>
            </RankingList>
          </>
